...
// Phase-13 + Phase-17: CAP summary card — uses getStats() single-pass query
...
} from '../../src/repositories/CorrectiveActionRepository';

const EMPTY: CapStats = {
  open: 0, inProgress: 0, overdue: 0,
  resolved: 0, total: 0, nearDeadlineCount: 0,
};
...
const pbStyles = StyleSheet.create({
  track: {
    height: 10, backgroundColor: '#eee', borderRadius: 5,
    overflow: 'hidden', marginTop: 10, marginBottom: 4, position: 'relative',
  },
  fill:  { position: 'absolute', top: 0, left: 0, bottom: 0, borderRadius: 5 },
  label: {
    position: 'absolute', right: 6, top: -1,
    fontSize: 9, fontWeight: '700', color: '#555', lineHeight: 12,
  },
});
...
export default function CapStatsWidget() {
  const [stats, setStats] = useState<CapStats>(EMPTY);
  const router            = useRouter();

  const load = useCallback(async () => {
    try {
      // Phase-17: single-pass aggregation — replaces getAll() + 4 filter passes
  ...
  // Hide when there are no CAP tasks at all
    ...
    <View style={widgetStyles.card}>
      ...
      <View style={widgetStyles.statRow}>
        ...
        <StatCol
          value={stats.overdue}
          label="متأخر"
          bg={stats.overdue > 0 ? '#ffebee' : '#f5f5f5'}
          fg={stats.overdue > 0 ? '#c62828' : '#999'}
        />
...
const widgetStyles = StyleSheet.create({
  card: {
    ...
    borderRadius: 12, marginHorizontal: 12, marginBottom: 12,
    padding: 14, elevation: 3,
