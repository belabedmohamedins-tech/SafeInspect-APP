...
## D0. Additional real-time re-checks (F-01, F-05) + one new observation
...
- **F-05** (prod API URL falls back to `localhost`): re-fetched `serverAuth.ts` in full just now — `getApiUrl()` still falls back to `'http://localhost:3000'` unconditionally. Confirmed still live. **New observation while re-reading this file:** it now contains a full server-side approval workflow (`approveInspection()` / `rejectInspection()`, `POST /inspections/:id/approve`/`/reject`) that didn't come up in earlier phases. **Not yet checked:** whether the mobile-local `ApprovalRepository.approve()` (the one behind F-11) actually calls this server endpoint, or whether local and server approval state can drift apart. Worth a look when F-11 is fixed, since it may affect how the immutability guard should work (local-only vs. server-confirmed approval).
...
6. **F-11** (approval immutability) — needs product sign-off on override workflow before implementation; do after the above so the "legally defensible record" story is consistent end-to-end.
7. **F-14** (status label + completion-rate reconciliation) — moderate, spans multiple files.
8. **F-09** (autosave) — independent, can slot in anytime.
9. **F-17** (server schema mapping) — do before enabling real sync, not urgent otherwise.
10. **F-16 and remaining legal citations** — route to whoever has primary legal-text access; not a code task.
...
**Real-time coverage: 13 of 17 findings re-fetched and reconfirmed against the live repository in this final pass. The 4 not re-fetched (F-02, F-03, F-06, F-16) are either purely cosmetic, already-resolved/informational, or legal-citation items whose truth doesn't depend on code commits.**
