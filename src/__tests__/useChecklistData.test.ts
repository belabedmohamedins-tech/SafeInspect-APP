...
describe('useChecklistData', () => {
  describe('initial load \u2014 new inspection (no draftId)', () => {
    it('loads default criteria when no activity specified', async () => {
      ...
      expect(result.data[0].complianceStatus).toBe('not-evaluated');
    });
  ...
  describe('initial load \u2014 draft (with draftId)', () => {
    ...
    it('loads existing draft items from repository', async () => {
      ...
      expect(result.data).toHaveLength(1);
      expect(result.data[0].complianceStatus).toBe('non-compliant');
    });
  ...
  describe('item handlers', () => {
    it('handleStatusChange updates the item status', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => { result.handleStatusChange('c1', 'compliant'); });
      expect(result.data.find(i => i.id === 'c1')?.complianceStatus).toBe('compliant');
    });

    it('handleCommentChange updates the item comment', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => { result.handleCommentChange('c2', 'needs repair'); });
      expect(result.data.find(i => i.id === 'c2')?.comment).toBe('needs repair');
    });

    it('handlePhotoTake adds a photo URI to the item', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => { result.handlePhotoTake('c1', 'file:///photo1.jpg'); });
      const item = result.data.find(i => i.id === 'c1')!;
    ...
    it('handlePhotoTake accumulates multiple photos', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => { result.handlePhotoTake('c1', 'file:///p1.jpg'); });
      act(() => { result.handlePhotoTake('c1', 'file:///p2.jpg'); });
      expect(result.data.find(i => i.id === 'c1')?.photos).toHaveLength(2);
    });

    it('keeps original photoUri when second photo is added', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => { result.handlePhotoTake('c1', 'file:///first.jpg'); });
      act(() => { result.handlePhotoTake('c1', 'file:///second.jpg'); });
      expect(result.data.find(i => i.id === 'c1')?.photoUri).toBe('file:///first.jpg');
    });
  });
  ...
  describe('derived values', () => {
    ...
    it('evaluatedItems is 0 on fresh load', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      expect(result.evaluatedItems).toBe(0);
    });
    ...
    it('progressPercent is 100 when all items are evaluated', async () => {
      ...
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => {
        result.handleStatusChange('c1', 'compliant');
        result.handleStatusChange('c2', 'non-compliant');
      });
    ...
    it('sections groups items by axis', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      expect(Object.keys(result.sections)).toContain('Axis A');
      expect(Object.keys(result.sections)).toContain('Axis B');
    });
  });
  ...
  describe('handleFinish', () => {
    it('shows alert when completion rate is below 85%', async () => {
      const { result } = renderHook(() => useChecklistData(BASE_PARAMS), { wrapper });
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => { result.handleStatusChange('c1', 'compliant'); }); // 1/2 = 50%
      await act(async () => { await result.handleFinish(); });
    ...
    it('shows alert when high-severity non-compliant item has no photo', async () => {
      ...
      await waitFor(() => expect(result.isLoading).toBe(false));
      act(() => {
        result.handleStatusChange('c1', 'non-compliant'); // high severity, no photo
        result.handleStatusChange('c2', 'compliant');
      });
      await act(async () => { await result.handleFinish(); });
