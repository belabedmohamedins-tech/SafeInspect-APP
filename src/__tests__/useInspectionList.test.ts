...
describe('useInspectionList', () => {
  ...
  it('loads inspections on focus', async () => {
    ...
    const { result } = renderHook(() => useInspectionList(), { wrapper });
    await waitFor(() => expect(result.totalCount).toBe(2));
  });
  ...
  describe('filtering', () => {
    ...
    it('shows all inspections with activeFilter=all', async () => {
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.filtered).toHaveLength(2));
    });

    it('filters to completed only', async () => {
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.totalCount).toBe(2));
      act(() => { result.setActiveFilter('completed'); });
      expect(result.filtered).toHaveLength(1);
      expect(result.filtered[0].id).toBe('c1');
    });

    it('filters to in-progress only', async () => {
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.totalCount).toBe(2));
      act(() => { result.setActiveFilter('in-progress'); });
      expect(result.filtered).toHaveLength(1);
      expect(result.filtered[0].id).toBe('p1');
    });

    it('filters by facilityName search query', async () => {
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.totalCount).toBe(2));
      act(() => { result.setSearchQuery('alpha'); });
      expect(result.filtered).toHaveLength(1);
      expect(result.filtered[0].id).toBe('c1');
    });
    ...
    it('filters by facilityAddress search query', async () => {
      ...
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.totalCount).toBe(1));
      act(() => { result.setSearchQuery('paix'); });
      expect(result.filtered).toHaveLength(1);
    });
    ...
    it('returns empty when search query matches nothing', async () => {
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.totalCount).toBe(2));
      act(() => { result.setSearchQuery('zzznomatch'); });
    ...
    it('sorts by date descending (newest first)', async () => {
      ...
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.filtered).toHaveLength(2));
      expect(result.filtered[0].id).toBe('new');
      expect(result.filtered[1].id).toBe('old');
    });
  });
  ...
  describe('deleteInspection', () => {
    it('shows a confirmation alert when called', async () => {
      ...
      const { result } = renderHook(() => useInspectionList(), { wrapper });
      await waitFor(() => expect(result.totalCount).toBe(1));
      await act(async () => { await result.deleteInspection('insp-1'); });
    ...
    it('calls InspectionRepository.delete and removes item on confirm', async () => {
      ...
      await act(async () => { await result.deleteInspection('del-1'); });
      ...
      expect(result.filtered.find(i => i.id === 'del-1')).toBeUndefined();
    });
    ...
    it('handles delete repository error gracefully', async () => {
      ...
      await act(async () => { await result.deleteInspection('err-1'); });
      const buttons: any[] = (mockAlert.mock.calls[0] as any)[2];
      const destructive = buttons.find((b: any) => b.style === 'destructive');
      await act(async () => { await destructive.onPress(); });
