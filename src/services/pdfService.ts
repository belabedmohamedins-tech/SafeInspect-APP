...
function statusBg(status: InspectionItem['complianceStatus']): string {
  switch (status) {
    case 'compliant':     return '#d4edda'; // soft green
    case 'non-compliant': return '#f8d7da'; // soft red
    case 'na':            return '#e2e3e5'; // grey
    default:              return '#fff3cd'; // amber — not evaluated
  }
}
...
<body>
  ...
  <!-- Indicator breakdown (only when score is available) -->
