...
export default function ReportsScreen() {
  ...
  const [sheetInspectionId, setSheetInspectionId] = useState<string | undefined>(undefined);
  ...
  const { filteredInspections, inspectionsWithViolations, totalViolationCount } = useMemo(() => {
    ...
    const withViolations = filtered.filter(ins =>
      ins.items.some(i => i.complianceStatus === 'non-compliant'),
    );
    const totalViolations = withViolations.reduce(
      (acc, ins) => acc + ins.items.filter(i => i.complianceStatus === 'non-compliant').length,
      0,
    );
    return {
    ...
    return (
      <Swipeable renderRightActions={() => renderRightActions(item.id)}>
        <TouchableOpacity
          style={styles.card}
          onPress={() =>
            router.push({ pathname: '/reports/[id]', params: { id: item.id } })
          }
        >
          ...
          <View style={styles.actionsRow}>
            ...
            <TouchableOpacity
              ...
              onPress={() => exportInspectionPDF(item)}
            >
            ...
            <TouchableOpacity
              style={styles.actionBtn}
              onPress={() => exportInspectionCSV(item)}
            >
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
        ...
        {(Object.keys(FILTER_LABELS) as FilterStatus[]).map(f => (
          <TouchableOpacity
            ...
            style={[
              styles.filterBtn,
              filterStatus === f &&
                (f === 'violations' ? styles.filterBtnViolations : styles.filterBtnActive),
            ]}
      ...
      />
      {/* Phase-11/12: CAP deadline sheet — uses current inspectionId-based API */}
      <CapFollowUpSheet
        inspectionId={sheetInspectionId}
