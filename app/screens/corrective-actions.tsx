...
// Phase-12: Corrective Action Tracker — full list, status cycling, filters
...
function relativeDeadline(isoDate: string): { text: string; urgent: boolean } {
  ...
  return {
    text: new Date(isoDate).toLocaleDateString('ar-DZ', { day: 'numeric', month: 'short', year: 'numeric' }),
    urgent: false,
  };
}
  ...
  return (
    <Swipeable renderRightActions={renderRight}>
      <View style={cardStyles.card}>
        {/* Top row: facility + severity chip */}
        ...
        <View style={cardStyles.bottomRow}>
          ...
          {/* Tappable status chip — cycles to next status */}
          <TouchableOpacity
            style={[cardStyles.statusChip, { backgroundColor: statusMeta.bg }]}
            onPress={() => nextStatus && onStatusCycle(action.id, nextStatus)}
            disabled={!nextStatus}
            accessibilityLabel={nextStatus ? `تغيير الحالة إلى ${STATUS_META[nextStatus].label}` : 'مغلق'}
          >
            <Text style={[cardStyles.statusText, { color: statusMeta.fg }]}>
              {statusMeta.label}
            </Text>
            {nextStatus && (
              <FontAwesome name="angle-left" size={11} color={statusMeta.fg} style={{ marginTop: 1 }} />
            )}
          </TouchableOpacity>
        </View>
      </View>
...
export default function CorrectiveActionsScreen() {
  ...
  const { displayed, overdueCount } = useMemo(() => {
    ...
    return { displayed: list, overdueCount: overdue };
      ...
      <View style={screenStyles.tabRow}>
        {(Object.keys(FILTER_LABELS) as FilterTab[]).map(tab => (
          <TouchableOpacity
            ...
            style={[
              screenStyles.tab,
              filterTab === tab && (tab === 'overdue'
                ? screenStyles.tabActiveOverdue
                : screenStyles.tabActive),
            ]}
