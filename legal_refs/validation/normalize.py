...
Slicing design note (2026-09-02):
  extract_articles() in index_articles.py slices at m.start() — the position
  of the \n immediately before each article marker. Every article slice
  therefore begins with \n, not with * or #. Paired bold/heading markers
  (**Art. 2.**) both land inside the same slice and are correctly handled by
  _RE_BOLD_ITALIC. No orphan-prefix artifact is produced under this design;
  the former _strip_orphan_prefix() safety net was dead code (^ anchor at
  position 0 never saw * because position 0 is always \n) and has been
  removed. If the slicing design ever changes back to m.end(), this
...
def normalize_md(text: str) -> str:
    ...
    Strips Markdown syntax, normalises unicode, collapses whitespace.
    Call this PER-ARTICLE after slicing, never on the whole file
    (would blind the boundary detector to Format A markers).
    """
